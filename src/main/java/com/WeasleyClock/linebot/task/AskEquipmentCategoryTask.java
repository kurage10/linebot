...
import com.weasleyclock.linebot.service.GrpcDisasterPreventionService;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.beans.factory.annotation.Autowired;

@LinebotTask(name = "ASK_EQUIPMENT_CATEGORY")
public class AskEquipmentCategoryTask extends BaseTask {
    private final Logger LOGGER = LoggerFactory.getLogger(AskEquipmentCategoryTask.class);
    public AskEquipmentCategoryTask(){}

    // private GrpcDisasterPreventionService service = new GrpcDisasterPreventionService();
    @Autowired
    private GrpcDisasterPreventionService service;

    @Override
    public Message start(TaskRequestDto request) {
        LOGGER.info("=== Start Ask Equipment Category ===");
        service.echoService();
        service.insertEquipment("test", "test", "test");
        // 仮に後続のタスクを続けるためにメッセージを返す。
        return createDinnerConfirmMessage();
    }

    @Override
    public WellKnownTaskBranchCode execute(TaskRequestDto request) {
        LOGGER.info("=== Execute Ask Equipment ===");
